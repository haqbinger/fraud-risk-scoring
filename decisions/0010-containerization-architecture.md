...
An unpinned `mlflow` package resolved to an invalid placeholder package in this project, failing the Linux build before the app ever started. Exact pinning avoids that drift and reproduces the environment tested locally.

We will also copy the entire `sql/` directory into the container image with:

- `COPY sql/ ./sql/`

The API reads `sql/prediction_logs.sql` at startup to create the `prediction_logs` table, so it has to ship inside the image. SQL DDL stays in `sql/` and the app executes it from there, rather than inlining DDL inside Python.
